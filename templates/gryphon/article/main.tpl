	...
	<div class="span8">
		...
		<article>
			...
			{% else %}
				...
				<div class="related-articles hidden-phone">
					<h3>Related Articles</h3>

					{% for article in related.shift(2) %}
						<article>
							<h4><a href="{{ article.url }}">{{ article.headline }}</a></h4>
							<img src="{{ article.media.grab('type','image')[0].urlThumbnail }}">
							<div>
								<aside>
									By: <span class="author">{{ article.authors.splat('name')|join(', ') }}</span> |
									<span>{{ article.created|date("m/d/y") }}</span>
								</aside>
