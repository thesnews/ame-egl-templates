...
<script type='text/javascript' src='http://adserver.thesne.ws/openx/www/delivery/ag.php'></script>
		...
		<div class="grid_5 alpha">
			...
			{% if images.length %}
				...
				{% set next = images[1] %}
				<div class="inline_gallery housing_gallery" id="gallery_1" style="width:350px;">
		...
		new Rose.ui.inlineGallery('gallery_1', {
			...
			onChange: function(data) {
				...
				this.container.getElements('.previous').set('href',
					prev.get('href'));
				var img = this.container.getElements('.photo').pop();

				if( data.dimensions.width > 300 ) {
					img.setStyles({
						width: 300,
						height: (300*data.dimensions.height)/data.dimensions.width
					});
				}
